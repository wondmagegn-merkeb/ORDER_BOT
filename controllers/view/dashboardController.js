...
exports.showDashBoard = async (req, res, next) => {
  try {
    ...
    const deliciousCount = await Order.count({
      where: { feedback: "delicious" },
    });
    ...
    const safeAvgOrderValue = avgOrder[0]?.avgOrderValue
      ? Number(avgOrder[0].avgOrderValue).toFixed(2)
      : 0;

    const minOrderValueDelivered =
      (await Order.min("totalPrice", {
        where: {
          status: "delivered",
        },
      })) || 0;

    const maxOrderValueDelivered =
      (await Order.max("totalPrice", {
        where: {
          status: "delivered",
        },
      })) || 0;
    ...
    const topUsers = await User.findAll({
      attributes: [
        ...
        [
          literal(
            "(SELECT COUNT(*) FROM `orders` AS `Order` WHERE `Order`.`createdBy` = `User`.`userId` AND `Order`.`deletedAt` IS NULL)"
          ),
          "orderCount",
        ],
      ],
    ...
    const safeTopUsers =
      topUsers.length > 0
        ? topUsers
        : [{ userId: "N/A", fullName: "No data", orderCount: 0 }];

    const usersWithOrders =
      (await Order.count({ distinct: true, col: "userId" })) || 0;
    ...
    // Get TOP 5 ITEMS for week (extract from items JSON array)
    const { sequelize } = require("../../config/db");
    const topItemsWeek = await sequelize.query(
      `
  SELECT 
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodId')) as foodId,
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodName')) as foodName,
    SUM(JSON_EXTRACT(item.value, '$.quantity')) as totalOrdered
  FROM orders,
  JSON_TABLE(items, '$[*]' COLUMNS (
    value JSON PATH '$'
  )) as item
  WHERE orders.createdAt BETWEEN :startOfWeek AND :endOfWeek
  GROUP BY foodId, foodName
...
`,
    ...
    const topItemsMonth = await sequelize.query(
      `
  SELECT 
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodId')) as foodId,
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodName')) as foodName,
  ...
  WHERE orders.createdAt BETWEEN :startOfMonth AND :endOfMonth
  GROUP BY foodId, foodName
  ORDER BY totalOrdered DESC
  LIMIT 5
`,
    ...
    const topItemsYear = await sequelize.query(
      `
  SELECT 
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodId')) as foodId,
    JSON_UNQUOTE(JSON_EXTRACT(item.value, '$.foodName')) as foodName,
  ...
  WHERE orders.createdAt BETWEEN :startOfYear AND :endOfYear
  GROUP BY foodId, foodName
  ORDER BY totalOrdered DESC
  LIMIT 5
`,
    ...
    // WEEK - Ensure it's an array before mapping
    const weekPieChartLabels = Array.isArray(topItemsWeek)
      ? topItemsWeek.map((item) => item.foodName)
      : [];
    const weekPieChartData = Array.isArray(topItemsWeek)
      ? topItemsWeek.map((item) => parseInt(item.totalOrdered))
      : [];

    // MONTH - Ensure it's an array before mapping
    const monthPieChartLabels = Array.isArray(topItemsMonth)
      ? topItemsMonth.map((item) => item.foodName)
      : [];
    const monthPieChartData = Array.isArray(topItemsMonth)
      ? topItemsMonth.map((item) => parseInt(item.totalOrdered))
      : [];

    // YEAR - Ensure it's an array before mapping
    const yearPieChartLabels = Array.isArray(topItemsYear)
      ? topItemsYear.map((item) => item.foodName)
      : [];
    const yearPieChartData = Array.isArray(topItemsYear)
      ? topItemsYear.map((item) => parseInt(item.totalOrdered))
      : [];
    ...
    const orderStatusThisWeek = await Order.findAll({
      attributes: ["status", [literal("COUNT(status)"), "statusCount"]],
      where: {
        createdAt: {
          [Op.between]: [startOfWeek, endOfWeek],
        },
